...
class EVSEDashboard {
    ...
    async init() {
        try {
            ...
            this.pullToRefresh = {
                ...
                indicator: document.querySelector('.pull-to-refresh-indicator'),
                startY: 0,
                isDragging: false,
                threshold: 80,
                listeners: null
    ...
    activatePullToRefresh() {
        if (window.EVSE_isDesktop()) {
            //console.log("🚫 Pull to Refresh DESATIVADO - Modo Desktop detectado");
            this.deactivatePullToRefresh(); // Garante que está desativado
            return;
        }

        const ptr = this.pullToRefresh;

        if (ptr.listeners) return; 

        const touchSurface = document.querySelector('.main-content'); 
        if (!touchSurface || !ptr.pageContainer || !ptr.indicator) {
            console.warn("PullToRefresh: Elementos essenciais não encontrados.");
            return;
        }
        ...
        const handleGestureStart = (e) => {
            ...
            isPointerDown = true;
            isDragging = false;
            ptr.pageContainer.style.transition = 'none';
        };

        const handleGestureMove = (e) => {
            if (!isPointerDown || ptr.isRefreshing) return;

            const currentY = e.touches ? e.touches[0].pageY : e.pageY;
            const diff = currentY - startY;

            // LÓGICA CORRIGIDA E ROBUSTA:
            // Usa ptr.element (document.documentElement) para a verificação de rolagem.
            // Só previne o padrão e ativa o 'isDragging' quando as duas condições são verdadeiras.
            if (ptr.element.scrollTop === 0 && diff > 0) {
                e.preventDefault(); // Assumimos o controle do gesto.

                isDragging = true;
                const moveY = Math.pow(diff, 0.85);
                ptr.pageContainer.style.transform = `translateY(${moveY}px)`;
                ptr.indicator.style.opacity = Math.min(1, moveY / ptr.threshold);
            }
        };
        ...
        const handleGestureEnd = (e) => {
            ...
            ptr.pageContainer.style.transition = 'transform 0.3s ease';
            
            const currentTransform = ptr.pageContainer.style.transform;
            const currentY = parseFloat(currentTransform.replace('translateY(', '').replace('px)', '')) || 0;

            if (currentY > ptr.threshold) {
                this.triggerContextualRefresh();
            } else {
                this.hideRefreshIndicator(); // Usando a função auxiliar para resetar
            }
        };

        ptr.listeners = {
            touchstart: handleGestureStart, touchmove: handleGestureMove, touchend: handleGestureEnd,
            mousedown: handleGestureStart, mousemove: handleGestureMove, mouseup: handleGestureEnd,
            mouseleave: handleGestureEnd
        };

        touchSurface.addEventListener('touchstart', ptr.listeners.touchstart, { passive: false });
        touchSurface.addEventListener('touchmove', ptr.listeners.touchmove, { passive: false });
        touchSurface.addEventListener('touchend', ptr.listeners.touchend);
        touchSurface.addEventListener('mousedown', ptr.listeners.mousedown);
        window.addEventListener('mousemove', ptr.listeners.mousemove, { passive: false });
        window.addEventListener('mouseup', ptr.listeners.mouseup);
        window.addEventListener('mouseleave', ptr.listeners.mouseleave);

        //console.log("Pull to Refresh ATIVADO");
    }

    /**
     * Desativa e remove todos os event listeners do "puxar para atualizar".
     * Chamado quando o usuário navega para uma página não atualizável.
     */
    deactivatePullToRefresh() {
        const ptr = this.pullToRefresh;
        // A verificação principal é se os listeners existem.
        if (!ptr.listeners) return;

        // 1. Identifica a mesma superfície de toque usada na ativação.
        const touchSurface = document.querySelector('.main-content');
        if (!touchSurface) {
            console.warn("Não foi possível encontrar a 'touchSurface' para desativar os listeners.");
            // Mesmo se não encontrar, continua para limpar os outros listeners.
        }

        //console.log("Desativando Pull to Refresh...");

        // 2. Remove os listeners dos elementos corretos.
        // É crucial que os nomes das funções (ptr.listeners.touchstart, etc.)
        // sejam exatamente os mesmos que foram usados no addEventListener.
        
        // Remove listeners de toque/mouse da superfície principal
        if (touchSurface) {
            touchSurface.removeEventListener('touchstart', ptr.listeners.touchstart);
            touchSurface.removeEventListener('touchmove', ptr.listeners.touchmove);
            touchSurface.removeEventListener('touchend', ptr.listeners.touchend);
            touchSurface.removeEventListener('mousedown', ptr.listeners.mousedown);
        }

        // Remove listeners de mouse/toque da janela (window)
        window.removeEventListener('mousemove', ptr.listeners.mousemove);
        window.removeEventListener('mouseup', ptr.listeners.mouseup);
        window.removeEventListener('mouseleave', ptr.listeners.mouseleave); // Se você adicionou este, remova também.

        // 3. Reseta o objeto de listeners para null.
        // Isso sinaliza que a funcionalidade está inativa e permite que
        // o activatePullToRefresh funcione novamente no futuro.
        ptr.listeners = null;

        //console.log("Pull to Refresh DESATIVADO.");
    }

    /**
     * Orquestrador: Verifica a página ativa e chama a atualização correta.
     */
    triggerContextualRefresh() {
        ...
        switch (pageId) {
            case 'page-home':
                refreshAction = this.refreshHomePage;
                break;
            case 'page-history': // Assumindo que você fará a troca de 'map' para 'history'
                refreshAction = this.refreshHistoryPage;
                break;
            case 'page-stats':
                refreshAction = this.refreshStatsPage;
                break;
            default:
                console.log(`A página '${pageId}' não é atualizável.`);
                this.hideRefreshIndicator();
                return;
        }

        console.log(`Acionando atualização para a página: ${pageId}`);
        const ptr = this.pullToRefresh;
        ptr.indicator.classList.add('loading');
        ptr.pageContainer.style.transform = `translateY(60px)`;
    ...
    refreshHomePage() {
        console.log("🔄 Atualizando página Início...");
        if (this.mqttClient && this.mqttClient.isConnected()) {
            this.mqttClient.disconnect();
        }
        ...
        delete this.homeChart;
    }

    refreshHistoryPage() {
        console.log("🔄 Atualizando página Histórico...");
        // Lógica futura para buscar dados do histórico aqui.
        setTimeout(() => { // Simulação de carregamento
            console.log("Histórico atualizado (simulação).");
            this.hideRefreshIndicator();
        }, 1500);
    }

    refreshStatsPage() {
        console.log("🔄 Atualizando página Estatísticas...");
        // Lógica futura para buscar dados de estatísticas aqui.
        setTimeout(() => { // Simulação de carregamento
            console.log("Estatísticas atualizadas (simulação).");
            this.hideRefreshIndicator();
        }, 2000);
    }

    /**
     * Função auxiliar para esconder o indicador e retornar a página ao normal.
     */
    hideRefreshIndicator() {
        const ptr = this.pullToRefresh;
        ptr.pageContainer.style.transform = 'translateY(0px)';
        setTimeout(() => {
            ptr.indicator.classList.remove('loading');
        }, 300);
        ptr.pageContainer.removeAttribute("style");
    }
